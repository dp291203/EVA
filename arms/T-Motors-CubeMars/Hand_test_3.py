...
def ramp_to_position(motor, target_deg, duration=0.5, steps=100):
    ...
    for pos in pos_array:
        motor.send_deg_command(pos, 2, 20, 3, 0)
        time.sleep(duration / steps)

    final_pos, vel, curr = motor.send_deg_command(target_deg, 2, 20, 3, 0)
    ...
    try:
        while True:
            try:
                ...
                for motor_id, target_deg in zip(motor_ids, target_pos_list):
