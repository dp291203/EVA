...
Kinematics::rpm Kinematics::calculateRPM(float linear_x, float linear_y, float angular_z)
{
    ...
    double transformationMatrix[4][3] = {{ 0,  1, 0.255},
                                         {-1,  0, 0.255},
                                         { 0, -1, 0.255},
                                         { 1,  0, 0.255}};
    ...
    //front-left motor
    rpm.motor4 = transformationMatrix[0][0] * x_rpm + transformationMatrix[0][1] * y_rpm + transformationMatrix[0][2] * tan_rpm;
    rpm.motor4 = constrain(rpm.motor4, -max_rpm_, max_rpm_);

    //front-right motor
    rpm.motor1 = -(transformationMatrix[1][0] * x_rpm + transformationMatrix[1][1] * y_rpm + transformationMatrix[1][2] * tan_rpm);
    rpm.motor1 = constrain(rpm.motor1, -max_rpm_, max_rpm_);

    //rear-left motor
    rpm.motor3 = transformationMatrix[3][0] * x_rpm + transformationMatrix[3][1] * y_rpm + transformationMatrix[3][2] * tan_rpm;
    rpm.motor3 = constrain(rpm.motor3, -max_rpm_, max_rpm_);

    //rear-right motor
    rpm.motor2 = -(transformationMatrix[2][0] * x_rpm + transformationMatrix[2][1] * y_rpm + transformationMatrix[2][2] * tan_rpm);
...
Kinematics::velocities Kinematics::getVelocities(float rpm1, float rpm2, float rpm3, float rpm4)
{
    ...
    average_rps_x = ((float)( rpm2 + rpm3) / 2) / 60.0; // RPM
    vel.linear_x = average_rps_x * wheel_circumference_; // m/s


    //convert average revolutions per minute in y axis to revolutions per second
    average_rps_y = ((float)(rpm1 + rpm4) / 2) / 60.0; // RPM
    vel.linear_y = average_rps_y * wheel_circumference_; // m/s



    //convert average revolutions per minute to revolutions per second
    average_rps_a = ((float)(rpm1 + rpm2 + rpm3 + rpm4) / 4) / 60.0;
